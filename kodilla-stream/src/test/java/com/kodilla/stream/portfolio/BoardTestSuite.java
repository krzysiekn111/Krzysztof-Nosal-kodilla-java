...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask() {
        ...
        long time = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(task -> task.getCreated().getDayOfYear())
        ...
        int daysNumber = LocalDate.now().getDayOfYear() * (int)numberOfTasksInProgress;
        int averageTime = (daysNumber - (int)time)/(int)numberOfTasksInProgress;
